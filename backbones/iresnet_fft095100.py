...
class FourierTransformLayer(nn.Module):
        ...
        for i in range(h):
            for j in range(w):
                ...
                if dist_h**2 + dist_w**2 <= 1:
                    ...
                    mask[i, j] = max(self.smoothness, distance)
    ...
    def forward(self, x):
        ...
        #print(self.learnable_param)
        # 应用平滑掩码
        #print(img_fft_shifted.device)
        #print(smooth_mask.device)
        #print(self.learnable_param)
        ...
        # 打印张量的维度
        print("Tensor shape:", self.learnable_param.shape)

        # 打印完整的张量内容
        print("Tensor data:\n", self.learnable_param)
        ...
        img_reconstructed = torch.fft.ifft2(img_enhanced).real  # 取实部
...
class IResNet(nn.Module):
    ...
    def __init__(self,
        ...
        # 其他初始化代码
        #self.fourier_transform_layer = FourierTransformLayer(0.95, 1, img_size)  # 初始化傅里叶变换层
        self.fourier_transform_layer = FourierTransformLayer(0.95, 1, 112)  # 初始化傅里叶变换层
    ...
    def forward(self, x):
        x = self.fourier_transform_layer(x)  # 进行傅里叶变换和高频增强
